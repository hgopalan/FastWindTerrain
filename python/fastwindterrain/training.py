...
#: Typical maximum friction velocity across the corpus, used only to
#: bring the ``ustar`` channel to the same order as the others.
USTAR_SCALE = 0.7


def frontal_area_index(terrain, direction_deg, dx, dy):
    """Frontal and plan area indices, the morphometric drag parameters.

    The surface-layer error correlates with the friction velocity of the
    case at -0.50 within site, and with the solid fraction at +0.49,
    while every LOCAL terrain descriptor tried -- slope, curvature, the
    along-wind gradient -- correlates at essentially zero. Both of the
    quantities that work are INTEGRAL measures of how much drag the
    terrain exerts, which is the role momentum thickness plays for a
    boundary layer that is not resolved.

    u* is an output of the solve and so is unavailable at inference.
    These are not: they follow from the terrain and the wind direction
    alone, which the network already receives, and they are what the
    urban-canopy literature (Grimmond and Oke; Macdonald) uses to predict
    drag from geometry.

    WHY THE FRONTAL INDEX IS EVEN IN DIRECTION. The obvious definition
    counts only windward rises, and is therefore larger looking upwind
    than downwind over asymmetric terrain. That would be wrong here. The
    reference solver is exactly ODD in the wind direction -- reversing it
    negates the solution to 0.00e+00 -- so it produces no lee-windward
    asymmetry at all, and half the corpus is generated by negation on
    that basis. Supplying an input which distinguishes upwind from
    downwind would let the network break a symmetry the data satisfies
    exactly. The magnitude of the along-wind gradient is used instead:
    it still separates a ridge crossed broadside from one crossed
    end-on, which is the drag signal wanted, while being unchanged under
    reversal.

    Returns ``(lambda_f, lambda_p)``.
    """
    h = np.asarray(terrain, dtype=np.float64)
    if h.ndim == 3:
        h = h[0]
    th = np.radians(float(direction_deg))
    wx, wy = -np.sin(th), -np.cos(th)     # unit vector the wind blows toward
    gy, gx = np.gradient(h, dy, dx)
    along = gx * wx + gy * wy             # rise per metre along the flow
    lam_f = float(np.abs(along).mean())   # even under a 180 degree reversal
    lam_p = float((h > np.median(h)).mean())
    return lam_f, lam_p
...
def make_input(arrays, direction_deg, dx, dy, scale=TERRAIN_SCALE_M,
    ...
    if drag:
        # Two constant planes: the frontal and plan area indices. Unlike
        # ustar these need no solve, so a gain here is deployable.
        lf, lp = frontal_area_index(arrays["terrain"], direction_deg, dx, dy)
        chans += [np.full(ter.shape, np.float32(lf * 10.0), dtype=np.float32),
                  np.full(ter.shape, np.float32(lp), dtype=np.float32)]
    if ustar is not None:
        # A DIAGNOSTIC CHANNEL, NOT A DEPLOYABLE ONE. The maximum
        # friction velocity is an OUTPUT of the solve; at inference it
        # would have to be predicted. It is supplied here to test whether
        # the surface-layer error is limited by this information, since
        # u* correlates with the sign of that error at -0.50 within site
        # while no terrain quantity correlates at all. A gain means the
...
class LevelDataset:
    ...
    def __getitem__(self, i):
        ...
        us = None
        if self.ustar:
            # From the manifest, not from the field: this is the value the
            # solver's wall function actually used for this case.
